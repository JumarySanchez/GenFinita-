...
function extractGeneratedVideos(msg, history) {
	const videos = [];
	if (msg.role !== 'assistant') {
		return videos;
	}

	const generateVideoToolCall = msg.tool_calls?.find(toolCall => toolCall.function.name === 'generate_video');

	if (generateVideoToolCall) {
		const generateVideoToolCallResult = history.find(historyMessage => historyMessage.role === 'tool' && historyMessage.tool_call_id === generateVideoToolCall.id)?.content;
		if (generateVideoToolCallResult) {
			videos.push(generateVideoToolCallResult);
		}
	}
...
function useIntegratedAi() {
			...
			try {
				...
				const chatMessages = historyMessages
					...
					.map((msg) => {
						...
						const videos = [...(msg.videos || []), ...extractGeneratedVideos(msg, historyMessages)];
	...
	const handleSSEEvent = useCallback((parsed) => {
		...
		if (parsed.type === SSEEventType.ToolResult) {
			...
			const isVideoResult = parsed.data.tool_name === 'generate_video' && parsed.data.content;
			...
			if (isVideoResult) {
				setMessages((prev) => {
					...
					updated[updated.length - 1] = {
						...last,
						videos: [...(last.videos || []), parsed.data.content],
					};
